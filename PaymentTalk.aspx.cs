...
namespace WowkoolMini
{
    public partial class PaymentTalk : System.Web.UI.Page
    {
        ...
        private string prepareData()
        {
            ...
            string _orderNumber = "99293213-122";
            string[] _itemNames = {"充電器","耳機","運費","生日蛋糕","運費"};
            string[] _itemId = { "PROD_020202", "PROD_020201", "FEE_000002", "PROD_030302", "FEE_000002" };
            string[] _vendorId = { "shop123", "shop123", "shop123","66shopping","66shopping" };
            string[] _vendorName = { "商店123", "商店123", "商店123", "66小舖", "66小舖" };
            int[] _qtys = {1,1,1,1,1};
            double[] _amts = {890,1000,0,670,0};

            double _tax = 0,_amountSum=0;
            ...
            for (int i = 0; i < _itemNames.Length; i++)
            {
                postData.Append("&vendorId=").Append(Server.UrlEncode(_vendorId[i]))
                        ...
                        .Append("&tax=").Append(_qtys[i]*_amts[i]*0.05); //5% tax for example
                _amountSum += _qtys[i] * _amts[i];
            }
            postData.Append("&amountSum=").Append(_amountSum)
